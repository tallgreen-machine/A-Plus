        ...
        <div className="overflow-x-auto">
             <table className="min-w-full text-sm text-left">
                <thead className="bg-brand-surface border-b border-brand-border">
                    <tr>
                        <th scope="col" className="px-4 py-3 font-medium text-brand-text-secondary uppercase">Strategy Name</th>
                            ...
                            >
                                <td className="px-4 py-3 whitespace-nowrap font-semibold text-brand-text-primary">
                                    <div className="flex items-center gap-2">
                                        <span className="text-brand-text-secondary">
                                            {expandedRowId === strategy.id ? <ChevronUpIcon /> : <ChevronDownIcon />}
                                        </span>
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                    <span className={`px-2 py-1 text-xs font-bold rounded-full ${
                                        strategy.status === StrategyStatus.ACTIVE ? 'bg-green-500/10 text-green-400' : 'bg-yellow-500/10 text-yellow-400'
                                    }`}>
                                        {strategy.status}
                                    </span>
                                </td>
                                <td className={`px-4 py-3 whitespace-nowrap font-semibold ${strategy.totalPL >= 0 ? 'text-brand-positive' : 'text-brand-negative'}`}>
                                    {formatCurrency(strategy.totalPL)}
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">{strategy.winLossRatio.toFixed(2)}</td>
                                <td className="px-4 py-3 whitespace-nowrap">{strategy.totalTrades}</td>
                            </tr>
                            {expandedRowId === strategy.id && (
